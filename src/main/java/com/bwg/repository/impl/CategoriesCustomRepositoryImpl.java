...
public class CategoriesCustomRepositoryImpl implements CategoriesCustomRepository {
    ...
    @Override
    public Page<CategoriesModel> fetchCategoriesWithServicesAndTags(String search, Pageable pageable) {
        String baseSql = """
            SELECT 
        ...
        long total = countCategories(search);

        return new PageImpl<>(categoriesList, pageable, total);
    }

    private long countCategories(String search) {
        String countSql = """
