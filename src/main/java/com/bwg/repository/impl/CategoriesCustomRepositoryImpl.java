...
public class CategoriesCustomRepositoryImpl implements CategoriesCustomRepository {
    ...
    """;
        ...
        idNativeQuery.setParameter("search", (search == null || search.isBlank()) ? "" : search);
        idNativeQuery.setParameter("tagName", (tagName == null || tagName.isBlank()) ? "" : tagName);
        idNativeQuery.setParameter("limit", pageable.getPageSize());
        idNativeQuery.setParameter("offset", pageable.getOffset());

        List<Long> categoryIds = ((List<Object[]>) idNativeQuery.getResultList()).stream()
                .map(row -> ((Number) row[0]).longValue()) // row[0] = category_id
                .toList();
    ...
    """;
        ...
        Query nativeQuery = entityManager.createNativeQuery(dataQuery, Tuple.class);
    ...
    private List<CategoriesModel> mapTuplesToCategories(List<Tuple> tuples) {
        ...
        for (Tuple row : tuples) {
            ...
            if (row.get("service_id") != null) {
                ServicesModel service = CategoriesMapper.mapServiceFromTuple(row, categoryId);
